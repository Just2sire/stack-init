...
function getTypeClass(type: string) {
  ...
  if (["string","char","tinytext"].some(k => t.includes(k)))          return "si-type-string";
  if (["text","mediumtext","longtext"].some(k => t.includes(k)))      return "si-type-text";
  if (["integer","biginteger","smallinteger","int"].some(k => t.includes(k))) return "si-type-integer";
  if (["decimal","float","double"].some(k => t.includes(k)))          return "si-type-decimal";
  if (t.includes("boolean"))                                           return "si-type-boolean";
  if (["timestamp","date","datetime"].some(k => t.includes(k)))       return "si-type-timestamp";
  if (["foreignid","foreignuuid"].some(k => t.includes(k)))           return "si-type-foreignid";
...
export function ModelCard({ model, onEdit }: ModelCardProps) {
  ...
  return (
    <div className="si-model-card">
      {/* Header */}
      <div className="si-model-card-header" onClick={onEdit} style={{ cursor: "pointer" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <span style={{ fontSize: 11, color: "var(--text3)", fontFamily: "var(--font-space-grotesk)", fontWeight: 400 }}>
      ...
      <div style={{ padding: "12px 20px 16px" }}>
        {model.fields.slice(0, 5).map((f) => (
          <div key={f.name} className="si-field-row">
            <span style={{ flex: 1, color: "var(--text)", fontFamily: "var(--font-jetbrains-mono)" }}>{f.name}</span>
            <span className={`si-type-chip ${getTypeClass(f.type)}`}>{f.type}</span>
        ...
        {model.fields.length === 0 && (
          <p style={{ fontSize: 12, color: "var(--text3)", fontStyle: "italic" }}>No fields yet — click to edit</p>
        )}
      </div>
